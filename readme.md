...
语言模型根据输入问题生成回答。
评估（Evaluation）
问题与回答对通过某个函数、模型、人工反馈或它们的组合进行打分，得到一个标量值。
优化（Optimization）
使用当前模型与一个参考模型对回答进行对数似然计算，KL 散度用于约束更新幅度，防止模型梯度更新过大导致不稳定。
随后，采用 PPO 算法对活跃的语言模型进行训练。
...
Warmup 阶段（W）：通过逐渐升高学习率，让模型在初期收敛更稳定、避免训练初期不稳定带来的梯度爆炸。
Stable 阶段（S）：在较长的训练阶段保持学习率稳定，帮助模型充分挖掘数据特征。
Decay 阶段（D）：在后期逐步降低学习率，防止过拟合并让模型在收尾阶段平稳收敛。
...
你可以使用 inference.py 在本地进行简单推理测试，或编写自己的服务接口（如 Flask 或 FastAPI）来部署模型。
# 6. 后续计划
